    ...
    <template #no-data>
      <v-list-item>
        <v-list-item-title>
          ...
          <span v-else-if="!query">{{ $t('Enter city') }}</span>
...
export default {
  ...
  methods: {
    ...
    async getSuggestions() {
      ...
      this.isLoading = true
      try {
        this.suggestions = await this.GET_CITIES({ query: queryTrimmed, count: 15 })
      } catch (err) {
    ...
    onChange(form) {
      ...
      const { value } = this
      this.$emit('input', this.form.data.city)
      if (value !== this.form.data.city) {
